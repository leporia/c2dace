		...
		while(fileSize > 0)
		{
			...
			while (l > 31) {
				...
				while (cond_ipblock) {
					...
					while_cond1 = m_pos[0];
					while_cond1 |= m_pos[1] << 8;
					while_cond1 |= m_pos[2] << 16;
					while_cond1 |= m_pos[3] << 24;

					while (dv != while_cond1) {
						unsigned int diff;
						diff = ip_block - ii;
						ip_block += 1 + (diff >> 5);
					...
					else
					{
						m_off -= 0x4000;
						if (m_len <= 0x9) {
							op[0] = (0x10 | ((m_off >> 11) & 8) | (m_len - 2));
							op++;
						} else
						{
							m_len -= 0x9;
							op[0] = (0x10 | ((m_off >> 11) & 8));
							op++;
							while (m_len > 255)
							{
								m_len -= 255;
